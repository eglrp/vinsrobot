...
void loadImageList(char * imagePath, std::vector<ICell> &iListData)
{
    ...
    int j = 0;
    size_t comma = 0;
    //无符号整数类型,可存放对象的最大大小,包括数组.
    size_t comma2 = 0;
    ICell temp;    //ImageList
    //把说明先读掉
    getline(inf, line);
    ...
    while (!inf.eof())
    {
        ...
        stringstream ss;
        // implements input/output operations on memory
...
void loadIMUFile(char * imuPath, std::vector<ORB_SLAM2::IMUData> &vimuData)
{
    ...
    while (!inf.eof())
    {
        ...
        //新元素到vector的尾端,pop_back()是删除尾端元素
        j = 0;
    }
    //经过调试发现上面的程序多加了最后一行数据，这里去掉最后一行数据
    vimuData.pop_back();
...
int main(int argc, char **argv)
{
    ...
    //cv::initUndistortRectifyMap用于相机校正映射,获取畸变坐标映射矩阵mapx和mapy后，作为remap函数的输入，多次调用remap函数进行畸变校正。
    ...
    for (int j = 0; j < iListData.size() - 1; j++)
    {
        ...
        */
        ...
        //把fullpath所指内存区域的前500个字节设置成0.
        memset(fullPath, 0, 500);
        memset(fullPathR, 0, 500);

        //双目矫正.CV_INTER_LINEAR: 非整数像素坐标插值标志. 这里给出的是默认值(双线性插值).
        //remap重映射，就是把一幅图像中某位置的像素放置到另一个图片指定位置的过程。M1l为X坐标重映射参数.
