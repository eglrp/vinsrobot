...
{
    ros::init(argc, argv, "ros_vio");
    ros::start();

    if(argc != 3)
    {
        cerr << endl << "Usage: rosrun modelx ros_vio path_to_vocabulary path_to_settings" << endl;
        ros::shutdown();
        return 1;
    }
    ...
    {
        imagesub = nh.subscribe(config._imageLeftTopic, /*200*/ 2, &ORBVIO::MsgSynchronizer::imageleftCallback, &msgsync);
